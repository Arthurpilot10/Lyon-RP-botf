...
// CONFIG IDs
// ═══════════════════════════════════════════════════════════════════════════════
const GUILD_ID         = '1515437067474042880';
const PANEL_CHANNEL_ID = '1515392913633185844';
const ROLE_STAFF_ID    = '1515096986926186646';
...
async function startForm(channel, member, type) {
  ...
  await channel.send({ content: `<@${member.user.id}> <@&${ROLE_STAFF_ID}>`, embeds: [welcomeEmbed] });
...
async function closeTicket(channel, closedBy) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Ce ticket a été fermé par <@${closedBy.user.id}>.\n` +
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isButton() && interaction.customId.startsWith('open_')) {
      ...
      try {
        const existing = await db.collection('tickets')
          .where('plaignant.discord_id', '==', member.user.id)
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_close') {
      ...
      const isCreator = sessions.get(interaction.channelId)?.userId === member.user.id;
